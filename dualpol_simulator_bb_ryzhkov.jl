...
sig = deg2rad(10.0)
r = exp(-2.0*sig^2)
A1 = (1.0/4.0)*(1.0+r)*(1.0+r)
A2 = (1.0/4.0)*(1.0-r)*(1.0-r)
A3 = ( 3.0/8.0 + (1.0/2.0)*r + (1.0/8.0)*r^4 )^2
A4 = ( 3.0/8.0 - (1.0/2.0)*r + (1.0/8.0)*r^4 )*( 3.0/8.0 + (1.0/2.0)*r + (1.0/8.0)*r^4 )
...
function calc_radar_variables(N0,lambda)
  ...
  for i in [1:17]
    ...
    elseif (i%2 == 0)
      ...
      zv += ( abs2(s_amp[i,1]) - 2*real( conj(s_amp[i,2])*(s_amp[i,2] - s_amp[i,1]) )*A1 + abs2( s_amp[i,2] - s_amp[i,1])*A3 )*N*intcoeff
